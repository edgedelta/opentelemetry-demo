...
namespace
{
  ...
  class CurrencyService final : public oteldemo::CurrencyService::Service
  {
                                  ...
                                  GetSupportedCurrenciesResponse *response) override
    {
      ...
      // Extract X-Service-Name from client headers
      const auto &metadata = context->client_metadata();
      auto x_service_name_it = metadata.find("X-Service-Name");
      if (x_service_name_it != metadata.end())
      {
        std::string client_service_name = std::string(x_service_name_it->second.data(), x_service_name_it->second.length());
        // Add net.peer.name from X-Service-Name
        span->SetAttribute("net.peer.name", client_service_name);
        logger->Info("X-Service-Name received: " + client_service_name + " for conversion amd net.peer.name set as " + client_service_name);
      }
